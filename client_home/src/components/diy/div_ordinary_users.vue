<template>
	<div class="diy_details_box diy_div_ordinary_users">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/ordinary_users/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "ordinary_users_id",
					source_id: this.obj["ordinary_users_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/ordinary_users/set?ordinary_users_id=' + _this.obj["ordinary_users_id"], {
								praise_len
							}, (res) => {
								if(res.result){
...
.rich_text .diy_html h3{
	font-size: var(--font_base)!important;
	color: #333!important;
