<template>
	<div class="diy_details_box diy_div_study_room_administrator">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "study_room_administrator_id",
					source_id: this.obj["study_room_administrator_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/study_room_administrator/set?study_room_administrator_id=' + _this.obj["study_room_administrator_id"], {
								praise_len
							}, (res) => {
								if(res.result){
