<template>
	<div class="diy_details_box diy_div_use_seat_information">
		<div class="warp">
			<div class="container">
		...
		data() {
			return {
				...
				itemList: [
						...
						{
							title: "管理员确认",
							name: "administrator_confirmation",
							type: "文本"
						},
				],
				richList: [
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "use_seat_information_id",
					source_id: this.obj["use_seat_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/use_seat_information/set?use_seat_information_id=' + _this.obj["use_seat_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
