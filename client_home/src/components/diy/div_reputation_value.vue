<template>
	<div class="diy_details_box diy_div_reputation_value">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "reputation_value",
					source_field: "reputation_value_id",
					source_id: this.obj["reputation_value_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/reputation_value/set?reputation_value_id=' + _this.obj["reputation_value_id"], {
								praise_len
							}, (res) => {
								if(res.result){
