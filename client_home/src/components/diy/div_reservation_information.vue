<template>
	<div class="diy_details_box diy_div_reservation_information">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "reservation_information_id",
					source_id: this.obj["reservation_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/reservation_information/set?reservation_information_id=' + _this.obj["reservation_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
