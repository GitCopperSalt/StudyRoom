						...
						<div class="card">
							...
							<div class="form_register">
								<b-form>
									...
									<!--普通用户-->
									<form_ordinary_users v-if="form.user_group === '普通用户'" :form="form_sub"></form_ordinary_users>
									<!--自习室管理员-->
									<form_study_room_administrator v-if="form.user_group === '自习室管理员'" :form="form_sub"></form_study_room_administrator>
...
<script>
	...
	/* 自习室管理员 */
	import form_study_room_administrator from "@/components/diy/form_study_room_administrator";
				...
				if(!ret && user_group){
					if(user_group === "普通用户"){
						...
						if(!ret && !form_sub["user_name"]){
							...
							res = await this.$get("~/api/ordinary_users/count?", {"user_name": form_sub["user_name"]});
							if(res.result){
								ret = "用户名已存在!";
							}
						}
					...
					if(user_group === "自习室管理员"){
						var form_sub = Object.assign({} ,this.form_sub);
						if(!ret && !form_sub["merchant_account_number"]){
							ret = "商家账号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/study_room_administrator/count?", {"merchant_account_number": form_sub["merchant_account_number"]});
							if(res.result){
								ret = "商家账号已存在!";
							}
						}
